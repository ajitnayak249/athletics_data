...
def weight_vs_height(df, sport):
    ...
    athelets_df["Medal"].fillna("No Medal", inplace=True)
    if sport != "Overall":
...
def men_vs_women(df):
    ...
    men = athelets_df[athelets_df["Sex"] == "M"].groupby('Year').count()["Name"].reset_index()
    women = athelets_df[athelets_df["Sex"] == "F"].groupby("Year").count()["Name"].reset_index()
